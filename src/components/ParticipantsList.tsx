...
export const ParticipantsList: React.FC = () => {
  ...
  const exportToCsv = () => {
    const headers = [
      'Nome', 'CPF', 'Email', 'Telefone', 'Cidade', 'Estado', 'Tipo Inscrição', 
      'Status Pagamento', 'Data Inscrição', 'Confirmado Evento'
    ];
    
    const csvData = filteredParticipants.map(p => [
      p.nome, p.cpf, p.email, p.telefone, p.cidade, p.estado, 
    ...
    link.download = `participantes_evento_x_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
